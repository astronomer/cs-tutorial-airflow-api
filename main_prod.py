...
import json
import pandas as pd
from secrets import deployment_name, release_name, airflow_api_key

class AirflowAPI:
    def __init__(self):
        self.domain = f'https://{deployment_name}/{release_name}/airflow'
        self.headers = {
            "cache-control": "no-cache",
            "content-type": "application/json",
            "accept": "application/json",
            "authorization": airflow_api_key
        }
    ...
    def _toggle_pause(self, dag_id, is_paused):
        ...
        response = requests.patch(url=url, headers=self.headers, data=json.dumps(payload)).json()
        print(response)
    ...
    def _get_task_instances(self):
        ...
        payload = {}
        response = requests.post(url=url, headers=self.headers, data=json.dumps(payload)).json()
